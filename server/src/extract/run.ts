...
async function main(): Promise<void> {
  ...
  if (cmd === "extract") {
    ...
    if (!recordingId) {
      console.error("usage: run.ts extract <recording_id>");
      process.exit(1);
    }
    const result = await runExtraction(recordingId);
    console.log("");
    ...
    console.log(`moments inserted:         ${result.momentsInserted}`);
    console.log(`personnel-flagged count:  ${result.personnelAssessmentCount}`);
  ...
  console.error("usage: run.ts extract <recording_id> | verify <recording_id> --run <uuid> --fathom <path>");
  process.exit(1);
}
